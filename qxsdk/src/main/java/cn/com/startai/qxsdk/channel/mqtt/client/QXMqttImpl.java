...
import cn.com.startai.qxsdk.channel.mqtt.event.QXTimerPingListener;
import cn.com.startai.qxsdk.channel.mqtt.event.QXTimerPingSender;
...
public class QXMqttImpl implements IQXMqtt, IQXNetworkListener {
    ...
    public void init() {
        if (isInit) {
            QXLog.e(TAG, "no need to repeat init qxmqtt");
            return;
        }
        isInit = true;
        if (mHandler == null) {
    ...
    @Override
    public void disconnect(final boolean isRelease) {
        connectState = ServerConnectState.DISCONNECTING;
        try {

            if (isRelease) {
                client.disconnectForcibly(200, 200);

                connectState = ServerConnectState.DISCONNECTED;
                QXLog.e(TAG, "client disconnected isRelease = " + isRelease);
...
//                });
    ...
    @Override
    public synchronized void publish(final BaseData mqttData, final IQXCallListener onCallListener) {
